...
import { Search, CheckSquare, Bell, Settings, Menu, Sun } from "lucide-react";
...
import { ClientSearchDialog } from "@/components/search/client-search-dialog";
...
export function Header({
  ...
  // Keyboard shortcut Ctrl+K or Cmd+K to open search dialog
  React.useEffect(() => {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-[#e2e5dc] bg-white/95 px-6 backdrop-blur-xs">
        ...
        <div className="flex items-center gap-4 flex-1 max-w-xl">
          ...
          <button
            ...
            className="group relative flex h-9 w-full max-w-md items-center justify-between rounded-lg border border-[#e2e5dc] bg-[#f8f9f6] pl-9 pr-3 text-left text-sm text-[#8c9485] transition-colors hover:border-[#ccd3c5] hover:bg-white focus:outline-hidden"
          >
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8c9485] group-hover:text-[#5f6b52]" />
            <span className="truncate">
              Search clients by name (e.g. Sondus)...
            </span>
            <kbd className="pointer-events-none hidden h-5 select-none items-center gap-1 rounded-sm border border-[#dce1d5] bg-[#f0f3ea] px-1.5 font-mono text-[10px] font-medium text-[#6b7562] sm:flex">
              ⌘K
            </kbd>
          </button>
        </div>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuLabel>
                <div className="flex flex-col space-y-0.5">
                  <p className="text-sm font-medium text-[#1e231d]">
                    {user.name}
                  </p>
      ...
      <ClientSearchDialog open={searchOpen} onOpenChange={setSearchOpen} />
    </>
  );
}
